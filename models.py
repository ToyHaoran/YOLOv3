...
from util.utils import build_targets, to_cpu, non_max_suppression
...
def create_modules(module_defs):
    """
    根据模块配置构造层块列表 Constructs module list of layer blocks
    :param module_defs: 模型参数，如[{'type': 'net', 'batch': '16', ...}, {'type': 'convolutional', 'batch_normalize': '1',...}]
    :return: (超参数hyperparams, 模块列表module_list)
    """
    hyperparams = module_defs.pop(0)  # 弹出模型超参数 {'type': 'net', 'batch': '16', ...}
    output_filters = [int(hyperparams["channels"])]  # 输入通道数，这里为RGB=3
    module_list = nn.ModuleList()  # 模块列表，按顺序搭建
    for module_i, module_def in enumerate(module_defs):
        # 第一个卷积组合 CBA 如{'type': 'convolutional', 'batch_normalize': '1', 'filters': '32',
        ...
        elif module_def["type"] == "route":  # 路由层完成通道拼接，输入1：26*26*256 输入2：26*26*128  输出：26*26*(256+128)
            layers = [int(x) for x in module_def["layers"].split(",")]
            filters = sum([output_filters[1:][i] for i in layers])
            modules.add_module(f"route_{module_i}", EmptyLayer())

        elif module_def["type"] == "shortcut":  # 残差连接，加法操作，输入1：26*26*128 输入2：26*26*128  输出：26*26*128
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, img_size=416):
        ...
        self.module_defs = parse_model_config(config_path)  # 读取模型配置文件，如yolov3.cfg
